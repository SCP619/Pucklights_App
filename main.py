...
def run_extraction(job_id: str, video_path: str) -> None:
    ...
    Creates its own YOLO instance per job for thread safety (CUDA context must
    belong to the thread that uses it).
    """
    ...
    # ── Load model on THIS thread so CUDA context is correct ─────────────────
    import torch
    device = 0 if torch.cuda.is_available() else "cpu"
    print(f"[{job_id[:8]}] Loading model on device={device} …")
    try:
        job_model = YOLO(MODEL_PATH)
    except Exception as e:
        jobs[job_id].update({"status": "failed", "error": f"Model load failed: {e}"})
        return
    ...
    print(f"[{job_id[:8]}] Starting track — {total_frames} frames @ {fps:.0f} fps, device={device}")

    try:
        results = job_model.track(
            source=video_path,
            stream=True,
            persist=True,
            conf=0.25,
        ...
        for i, r in enumerate(results):
            ...
            # ── Debug: log detections every 300 frames ────────────────────
            if i % 300 == 0:
                detected = [(job_model.names[int(b.cls[0])], round(float(b.conf[0]), 2))
                            for b in r.boxes]
                print(f"[{job_id[:8]}] frame {i}/{total_frames} — detections: {detected}")

            is_goal, _ = check_for_goal(r)
            buffer_start_frame = max(0, i - len(frame_buffer) + 1)

            if is_goal and goal_detected_cooldown == 0:
                print(f"[{job_id[:8]}] 🚨 GOAL at frame {i} ({i/fps:.1f}s)")
            ...
            if post_goal_frames > 0:
                ...
                if post_goal_frames == 0:
                    ...
                    save_highlight_with_audio(
                        ...
                        out_path=out_path,
                        source_video=video_path,   # file still exists at this point
        ...
        print(f"[{job_id[:8]}] ❌ Extraction error: {e}")
        return

    jobs[job_id]["status"] = "completed"
    print(f"[{job_id[:8]}] ✅ Done — {highlight_count} goal(s) found.")
